...
In a real life, i’d strongly recommend making refactoring in such systems to avoid this situation as it possible.
...
And when you request intern on the string, you get a reference from this pool if such string exist there
...
Also, there is a chance that a foreign code was synchronize on
...
In order to return a same instance of mutex on the each of requests with equal keys,
we will need to store the created mutexes.
If we will store these mutexes in the simple `HashMap`,
than a size of the map will increase as new keys appears.
And we don't have a tool to evaluate a time when a mutex not used anywhere.

In this case, we can use a `WeakReference` to save a reference to the mutex in the map, just when it uses.
In order to implement this behavior, we used the `WeakHashMap` data structure.
