...
до того как мы запустим тест. В такой ситуации легко оказаться, если вы исопльзуете TestContainers для запуска redis в тестах.
...
Аннотация `Testcontainers`, при помощи расширения для junit5, помогающая работать с контейнерами в тесте,
...
и добавить какое-нибудь расширение, стартующее redis-контейнер и проставляющее host/port на котором он запустился, перед запуском теста.
