...
до того как мы запустим тест. В такой ситуации легко оказаться, если вы например исопльзуете TestContainers для запуска redis в тестах.
...
Из-за таких особенностей приходится изворачиваться и в тестах писать свои ApplicationContextInitializer-ы или инициализировать эти свойства в static инициализаторах:
...
она ищет их в коде по наличию анотации `Container` и запускает перед выполнением теста.

Конечно, можно было сделать решение, заточенное специально для этой задачи
и добавить какое-нибудь расширение, проставляющее host/port на котором запустился redis-контейнер, перед запуском теста.
...
<2> прописываем хост/порт на котором стартовал контейнер в стандартные спринговые проперти
