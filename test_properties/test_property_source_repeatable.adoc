...
включения логирование. И это далеко не одна настройка, их нужно прописать порядка десяти штук:
...
все это даже будет работать, мы увидим в логах запрсоы и много полезной информации от hibernate:
...
над тестовым классом, а одну внутри своей мета-аннотации).
...
Тем более, немного погуглив я понял,
что не только мне нужно это решение.
...
уже не будет. Надо сказать большое спасибо комьюнити Spring Framework за такую оперативность и помощь.
