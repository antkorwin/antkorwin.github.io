= How to use JUnit5 in Spring Framework 4
...
To fix this you need to add Sam Brannen's dependency:
...
Make a new spring boot project with parent link to the 1.5.9:
...
<3> all dependencies which you need
<4> my custom junit5 extensions to work with a testcontainers and other useful things
<5> Sam Brannen's junit5 supports for SpringFramework4


Add one simple entity and repository:
...
Now we can write a JUnit5 test for this repository:
...
On this level, we can see how to use JUnit5 repeatable annotation `ExtendWith` to add multiple extension in one meta-annotation:
