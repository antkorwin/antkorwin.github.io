...
Parallel Execution of tests is experimental available from version 5.3.0-RC1 of JUnit.
...
And the last step to set up is the properties file `junit-platform.properties`,
...
Let's start with a simple example of one test class with two test-methods:
...
if we look at the console output, we see names of two different threads, it's good, parallel execution works.
...
----
<1> I used this assumption to make assertions only if the current configuration provides a parallel execution.
...
Add an one more test class and return to the question about a elapsed time:
...
----
<1> on the first call of the beforeAll method, we store current time in extension context storage(if it absent)
<2> store the number of all test classes in the context
<3> this is not clean construction but for our's testing purpose it's possible to use it for obtain a number of all test classes that's run.
...
It's a brilliant feature in JUnit5!

If you are still using junit4, think hard about migration.
