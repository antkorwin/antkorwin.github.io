...
Else if you select a `REMOTE` mode, you will need to setup the `repositoryRoot` where the stub runner will find a jar with contracts
...
let's run the next command under a directory with jars of contract stubs:
...
the stub runner will find a more closest variant of API which existed in the stub and shows it for you:
