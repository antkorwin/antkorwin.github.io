...
If we change a contract on the producer side, our consumer will not be able to get a data from producer:
...
In this article, we will consider how to prevent this situation by using the cloud contracts.
In order to do it, we will write contract tests and will consider how to delivery a contract to each of the services.
...
[source]
----
$ java -jar ../spring-cloud-contract-stub-runner-boot-2.0.1.RELEASE.jar --stubrunner.stubs-mode="local" --stubrunner.ids=com.a ntkorwin:prediction-service:+:stubs:7000
----
...
## How to use the Cloud Contracts with the remote repository and configure CI system.
...
All what we need is to run the next docker compose file:
