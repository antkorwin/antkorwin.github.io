...
For example, consider the business process that is known to most developers.
...
----
<1> to save a state machine we using the HashMap
...
In order to make several state machines, you need to use a name on the state machine factory declaration:
...
Now you can inject the state machine by using this name in qualifier:
...
So, you can use several state machines at the same time.
