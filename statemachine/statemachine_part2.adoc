...
## How to guarantee consistency of transactions within using of state machine

Often, we have a business domain model separately from the state machine.
In ours projects a state machines used just for organization of a business workflows.
We store an all domain-specific data in other object.
Usually, we store this model in its own database and operate with it by using transactions.

In this chapter we will consider how to safety work with a state machines.
And how to guarantee a consistency of a state machine and another systems.
